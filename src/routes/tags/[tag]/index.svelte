  ...
  export async function load({ page, fetch }: LoadInput) {
    try {
      ...
      const blogsByTag = blogs.filter((post: IBlog) =>
        !post.tags
          ? []
          : new RegExp(post.tags.join("|"), "i").test(
              convertToSentence(page.params.tag)
            )
      );
...
<div class="flex flex-col justify-center items-start max-w-2xl mx-auto mb-16">
  ...
  <p class="text-gray-600 dark:text-gray-400 mb-4">
    There {blogs.length > 1
      ? `are ${blogs.length} articles`
      : `is ${blogs.length} article`} that belongs to the tag
    {readableTag}. Articles which belongs only to the tag {readableTag} will appear
    here. A particular article may belong to multiple tags. Use the search below
    to filter by title.
  </p>
  <!-- Start: Search blogs -->
