...
type Join<T> = T extends [infer I, ...infer R]
  ? `${I & string} ${Join<R>}`
  : "";
...
// -----------------------------------------------------------------
export const cn = <T extends ClassNames[]>(
  ...args: T
): Join<{ [K in keyof T]: TypeofClassName<T[K]> }> =>
  args.reduce<any>((a, b) => {
    if (!b) return a;
    if (typeof b === "string") return (a + " " + b).trim();
    if (Array.isArray(b)) return cn(a, b[0] ? b[1] : b[2]);
    if (typeof b === "object")
      return cn(a, ...(Object.entries(b).map((x) => x.reverse()) as any[]));
    return a;
  }, "");
