...
{#each mixData as data}
  {#if mix && mix?.slug}
    <div class="mb-8 w-full border-b border-gray-100 dark:border-gray-800 pb-5">
      <a data-sveltekit:prefetch href={`/mixes/${mix.slug}`} class="w-full">
        ...
        <iframe
          ...
          frameborder="no"
          allow="autoplay"
          src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/{data.id}&color=%23ff5500&auto_play=false&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true&visual=false"
        />
        ...
        >
          ...
          ·
          <a
            href="https://soundcloud.com/loose-lips123/loose-lips-mix-series-306-ukaea"
            title="Loose Lips Mix Series - 306 - UKAEA"
            target="_blank"
            style="color: #cccccc; text-decoration: none;">{data.title}</a
          >
        </div>
      ...
      <p class="text-gray-600 dark:text-gray-400">{mix.description}</p>
      <p class="text-gray-600 dark:text-gray-400">{data.date}</p>
